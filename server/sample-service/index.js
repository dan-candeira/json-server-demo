...
module.exports = function (server, bodyParser) {
	try {
		...
		const json = {
			data: Array.from(Array(10).keys()).map((index) => ({
				id: index,
				nomeAluno: faker.name.fullName(),
				dataInicio: faker.date.between(),
				ativo: index % 2 == 0 ? true : false,
				avatar: faker.image.abstract(undefined, undefined, true),
			})),
		};
	...
	server.use(bodyParser);
	server.put('/students/:id', function (req, res) {
		const id = req.params.id;
	...
	server.delete('/students/:id', function (req, res) {
		const id = req.params.id;
